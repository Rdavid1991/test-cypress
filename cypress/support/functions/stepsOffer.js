...
import { createValues, editedValues } from "../../fixtures/offer/offerValues.json";
import { buttons, inputs, modals, stepsIdentify, check } from "../../fixtures/offer/offerElements.json";
...
export const stepsCrear = {
  ...
  step4: () => {
    ...
    cy.get(inputs.offerObjective).type(createValues.offerObjective, { force: true });

    cy.get(inputs.sinipCode).type("123456", { force: true });

    cy.get(inputs.offerDescription).type(createValues.offerDescription, { force: true });

    cy.get(inputs.budgetItem).type(createValues.budget, { force: true });

    cy.get(inputs.generalOrCentralProg).type(createValues.generalOrCentralProg, { force: true });

    cy.get(inputs.estimatedBudget).type(createValues.budget, { force: true });

    cy.get(inputs.decentralizedFunds).type(createValues.budget, { force: true });

    cy.get(inputs.otherSourcesOfFinancing).type(createValues.OtherSourcesOfFinancing, { force: true });
